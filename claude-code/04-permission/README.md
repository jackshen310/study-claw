...
在控制流分析中，权限系统不是“事后审计”，而是**工具执行前的同步门禁**，位置非常关键：

1. **工具调用解析后**：模型输出 tool_use，但真正执行前必须经过权限判断。
2. **规则裁决路径**：deny > ask > allow > 默认行为，决定是自动执行、询问用户还是直接拒绝。
3. **作用域影响控制流**：权限结果会改变后续轮次的 messages（例如拒绝会产生不同的 tool_result）。
4. **与 Hooks 协同**：PreToolUse / PermissionRequest 事件为权限决策提供扩展钩子。

这意味着权限系统不仅是安全机制，也是控制流中的关键分支点。
